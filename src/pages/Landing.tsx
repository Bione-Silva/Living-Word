    ...
    <div className="min-h-screen overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif", color: '#0F0A18' }}>
      ...
      >
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between gap-3">
          <span className="font-display text-lg tracking-tight" style={{ color: '#F5F3FF' }}>Living <span style={{ color: '#6D28D9' }}>Word</span></span>
          ...
          <button className="md:hidden p-1.5" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            {mobileMenuOpen ? <X className="h-5 w-5" style={{ color: '#F5F3FF' }} /> : <Menu className="h-5 w-5" style={{ color: '#F5F3FF' }} />}
          </button>
        </div>
        {mobileMenuOpen && (
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div>
              ...
              <h1 className="font-display text-[28px] sm:text-[42px] font-bold leading-[1.15] tracking-tight mb-5" style={{ color: '#F5F3FF' }}>
                {copy.hero.h1[lang]}
                <br />
            ...
            <div className="hidden lg:flex relative justify-center items-center">
                  ...
                  <div className="px-4 pb-5 pt-2 space-y-3" style={{ background: '#FFFFFF', minHeight: '400px' }}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex gap-1">
                        {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                          <span key={l} className="text-[7px] font-bold px-1.5 py-0.5 rounded" style={{ background: l === lang ? '#0F0A18' : 'rgba(15,10,24,0.1)', color: l === lang ? '#F5F3FF' : '#0F0A18' }}>{l}</span>
                        ))}
                      </div>
                    </div>
                    {/* Form fields */}
                    ...
                    <div className="rounded-lg p-2.5" style={{ background: '#fff', border: '1px solid rgba(15,10,24,0.08)' }}>
                      ...
                      <div className="space-y-1">
                        <div className="h-1.5 rounded w-full" style={{ background: '#FFFFFF' }} />
                        <div className="h-1.5 rounded w-5/6" style={{ background: '#F5F3FF' }} />
                        <div className="h-1.5 rounded w-4/5" style={{ background: '#F5F3FF' }} />
                        <div className="h-1.5 rounded w-2/3" style={{ background: '#F5F3FF' }} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="absolute left-0 bottom-12 z-20 flex gap-1.5 rounded-xl px-3 py-2 shadow-lg backdrop-blur-sm" style={{ background: 'rgba(224,231,255,0.92)', border: '1px solid rgba(109,40,217,0.2)' }}>
                {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                  <span key={l} className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ background: l === lang ? '#0F0A18' : 'rgba(15,10,24,0.1)', color: l === lang ? '#F5F3FF' : '#0F0A18' }}>{l}</span>
                ))}
              </div>
              {/* Glow */}
            ...
            {copy.problem.cards.map((c, i) => {
              ...
              return (
                <RevealOnScroll key={i} delay={i * 0.12}>
                  <div className="rounded-xl p-5 h-full" style={{ background: '#F5F3FF', border: '1px solid rgba(15,10,24,0.12)' }}>
      ...
      {/* ===== 4. HOW IT WORKS ===== */}
      <section id="how" className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#F5F3FF' }}>
        <div className="max-w-3xl mx-auto">
            ...
            {copy.how.steps.map((step, i) => (
              <RevealOnScroll key={i} delay={i * 0.18}>
                <div className="flex gap-5 py-5" style={{ borderBottom: i < 2 ? '1px solid rgba(15,10,24,0.1)' : 'none' }}>
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 font-display text-xl font-semibold" style={{ background: '#0F0A18', color: '#F5F3FF' }}>{i + 1}</div>
                  <div className="flex-1">
                    ...
                    <span className="inline-block text-[12px] font-medium px-3 py-1 rounded-full" style={{ background: '#FFFFFF', color: '#0F0A18' }}>{step.badge[lang]}</span>
                  </div>
                </div>
              </RevealOnScroll>
            ))}
          ...
          <RevealOnScroll delay={0.3}>
            <div className="max-w-2xl mx-auto">
              <div className="rounded-t-xl overflow-hidden" style={{ background: '#FFFFFF', border: '2px solid rgba(15,10,24,0.15)' }}>
                  ...
                  <div className="flex gap-5">
                    <div className="flex-1 space-y-2.5">
                      ...
                      <div className="flex gap-1.5 pt-1">
                        {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                          <span key={l} className="text-[7px] font-semibold px-2 py-0.5 rounded" style={{ background: l === lang ? '#0F0A18' : 'rgba(15,10,24,0.1)', color: l === lang ? '#F5F3FF' : '#0F0A18' }}>{l}</span>
                        ))}
                      </div>
                    ...
                    <div className="flex-1 hidden sm:block">
                      <div className="flex flex-wrap gap-1 mb-2">
                        {[
                          ...
                          <span key={i} className="text-[7px] font-semibold px-1.5 py-0.5 rounded" style={{ background: i === 0 ? '#0F0A18' : 'rgba(15,10,24,0.08)', color: i === 0 ? '#F5F3FF' : '#1E1240' }}>{tab}</span>
                        ))}
                      </div>
                      <div className="rounded-lg p-2.5 space-y-1.5" style={{ background: '#fff', border: '1px solid rgba(15,10,24,0.08)' }}>
                        <div className="h-2 rounded w-3/4" style={{ background: '#FFFFFF' }} />
                        <div className="h-1.5 rounded w-full" style={{ background: '#F5F3FF' }} />
                        <div className="h-1.5 rounded w-full" style={{ background: '#F5F3FF' }} />
                        <div className="h-1.5 rounded w-5/6" style={{ background: '#F5F3FF' }} />
                        <div className="h-1.5 rounded w-full" style={{ background: '#F5F3FF' }} />
                        <div className="h-1.5 rounded w-2/3" style={{ background: '#F5F3FF' }} />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <RevealOnScroll>
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 items-center">
              {/* Input card */}
              <div className="rounded-2xl p-6 text-center" style={{ background: '#F5F3FF', border: '1px solid rgba(15,10,24,0.12)' }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {copy.multiformat.outputs.map((out, i) => (
                  <div key={i} className="rounded-xl px-3 py-3 flex items-center gap-2 transition-all hover:scale-[1.02]" style={{ background: '#F5F3FF', border: '1px solid rgba(15,10,24,0.08)' }}>
      ...
      <section id="features" className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#F5F3FF' }}>
        <div className="max-w-4xl mx-auto">
      ...
      <section className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#0F0A18' }}>
        <div className="max-w-3xl mx-auto">
          <h2 className="font-display text-[22px] sm:text-[32px] font-semibold text-center mb-8 leading-tight" style={{ color: '#F5F3FF' }}>{copy.vs.h2[lang]}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      {/* ===== 8. TESTIMONIALS ===== */}
      <section className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#F5F3FF' }}>
        <div className="max-w-3xl mx-auto">
            ...
            {copy.testimonials.itemsByLang[lang].map((t, i) => (
              <div key={i} className="rounded-2xl p-6" style={{ background: '#FFFFFF', border: '1px solid rgba(15,10,24,0.08)', boxShadow: '0 1px 4px rgba(15,10,24,0.04)' }}>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-semibold shrink-0 tracking-wide" style={{ background: '#FFFFFF', color: '#0F0A18' }}>{t.initials}</div>
                  <div>
      ...
      <section className="py-14 sm:py-16 px-5 sm:px-8" style={{ background: '#FFFFFF' }}>
        <div className="max-w-4xl mx-auto">
          <div className="rounded-2xl p-6 sm:p-10" style={{ background: '#F5F3FF', border: '1px solid rgba(15,10,24,0.1)' }}>
            <div className="flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="flex-1">
                ...
                <Link to="/cadastro" className="inline-flex items-center gap-2 text-[14px] font-semibold px-5 py-2.5 rounded-lg transition-all hover:scale-[1.02]" style={{ background: '#0F0A18', color: '#F5F3FF' }}>
            ...
            {regionLoading ? (
              [0,1,2,3].map(i => (
                <div key={i} className="rounded-xl p-5 flex flex-col gap-3 animate-pulse" style={{ background: '#FFFFFF', border: '1px solid rgba(15,10,24,0.12)' }}>
                  <div className="h-4 w-16 rounded" style={{ background: '#FFFFFF' }} />
                  <div className="h-9 w-24 rounded" style={{ background: '#FFFFFF' }} />
                  <div className="h-3 w-20 rounded" style={{ background: '#FFFFFF' }} />
                  <div className="space-y-2 mt-4">
                    {[0,1,2,3].map(j => <div key={j} className="h-3 w-full rounded" style={{ background: '#FFFFFF' }} />)}
                  </div>
                  <div className="h-10 w-full rounded-lg mt-auto" style={{ background: '#FFFFFF' }} />
                </div>
              ))
            ) : (
              pricingPlans.map((plan, i) => (
                <RevealOnScroll key={i} delay={i * 0.1}>
                  <div className="rounded-xl p-5 flex flex-col h-full" style={{
                    background: plan.featured ? '#F5F3FF' : '#FFFFFF',
                    border: plan.featured ? '2px solid #0F0A18' : '1px solid rgba(15,10,24,0.12)',
                  }}>
                    {plan.featured && (
                      <span className="inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full mb-2 self-start" style={{ background: '#0F0A18', color: '#F5F3FF' }}>
                    ...
                    <span className="inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full mb-4 self-start" style={{ background: '#FFFFFF', color: '#0F0A18' }}>
                      {plan.capacity[lang]}
                    </span>
                    ...
                    <Link to={plan.planKey ? `/cadastro?plan=${plan.planKey}` : '/cadastro'} className="block text-center text-[14px] font-semibold py-3 rounded-lg transition-transform hover:scale-[1.02]" style={{
                      background: plan.featured ? '#0F0A18' : '#FFFFFF',
      ...
      {/* ===== 11. FAQ ===== */}
      <section className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#F5F3FF' }} itemScope itemType="https://schema.org/FAQPage">
      ...
      <section className="py-16 sm:py-20 px-5 text-center safe-area-bottom" style={{ background: '#0F0A18' }}>
        <div className="max-w-2xl mx-auto">
          <h2 className="font-display text-[24px] sm:text-[38px] font-semibold leading-tight mb-4" style={{ color: '#F5F3FF' }}>
          ...
          <Link to="/cadastro" className="inline-block text-[16px] font-semibold px-9 py-4 rounded-xl mb-4 transition-transform hover:scale-[1.02]" style={{ background: '#F5F3FF', color: '#0F0A18' }}>{copy.ctaFinal.cta[lang]}</Link>
          <br />
