...
function PWAFooterInstallButton({ lang }: { lang: L }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {showIosHint && (
        <p className="text-[11px] max-w-xs text-center leading-relaxed" style={{ color: 'rgba(224,231,255,0.55)' }}>
    ...
    <div className="min-h-screen overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif", color: '#0F0A18' }}>
      ...
      >
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center justify-between gap-3">
          <span className="font-display text-lg tracking-tight" style={{ color: '#E0E7FF' }}>Living <span style={{ color: '#6D28D9' }}>Word</span></span>
          <div className="hidden md:flex items-center gap-7">
            <button onClick={() => scrollTo('how')} className="text-[13px] font-medium transition-colors hover:text-[#6D28D9]" style={{ color: 'rgba(224,231,255,0.65)' }}>{copy.nav.how[lang]}</button>
            <button onClick={() => scrollTo('features')} className="text-[13px] font-medium transition-colors hover:text-[#6D28D9]" style={{ color: 'rgba(224,231,255,0.65)' }}>{copy.nav.features[lang]}</button>
            <button onClick={() => scrollTo('pricing')} className="text-[13px] font-medium transition-colors hover:text-[#6D28D9]" style={{ color: 'rgba(224,231,255,0.65)' }}>{copy.nav.plans[lang]}</button>
            <Link to="/login" className="text-[13px] font-medium" style={{ color: 'rgba(224,231,255,0.65)' }}>{copy.nav.login[lang]}</Link>
            ...
            {/* Language toggle */}
            <div className="flex items-center rounded-lg overflow-hidden" style={{ background: 'rgba(224,231,255,0.1)', border: '1px solid rgba(224,231,255,0.15)' }}>
              {(['PT', 'EN', 'ES'] as L[]).map((code) => (
                <button
                  ...
                  style={{
                    background: lang === code ? 'rgba(224,231,255,0.9)' : 'transparent',
                    color: lang === code ? '#0F0A18' : 'rgba(224,231,255,0.5)',
                  }}
                >
                  {code}
                </button>
          ...
          <button className="md:hidden p-1.5" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            {mobileMenuOpen ? <X className="h-5 w-5" style={{ color: '#E0E7FF' }} /> : <Menu className="h-5 w-5" style={{ color: '#E0E7FF' }} />}
          </button>
        </div>
        {mobileMenuOpen && (
          <div className="md:hidden px-5 pb-5 space-y-3" style={{ background: 'rgba(15,10,24,0.98)' }}>
            <button onClick={() => scrollTo('how')} className="block w-full text-left text-[14px] py-2" style={{ color: 'rgba(224,231,255,0.7)' }}>{copy.nav.how[lang]}</button>
            <button onClick={() => scrollTo('features')} className="block w-full text-left text-[14px] py-2" style={{ color: 'rgba(224,231,255,0.7)' }}>{copy.nav.features[lang]}</button>
            <button onClick={() => scrollTo('pricing')} className="block w-full text-left text-[14px] py-2" style={{ color: 'rgba(224,231,255,0.7)' }}>{copy.nav.plans[lang]}</button>
            <Link to="/login" className="block text-[14px] py-2" style={{ color: 'rgba(224,231,255,0.7)' }}>{copy.nav.login[lang]}</Link>
            ...
            <div className="flex items-center rounded-lg overflow-hidden w-fit" style={{ background: 'rgba(224,231,255,0.1)', border: '1px solid rgba(224,231,255,0.15)' }}>
              {(['PT', 'EN', 'ES'] as L[]).map((code) => (
                <button
                  key={code}
                  onClick={() => setLang(code)}
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div>
              ...
              <h1 className="font-display text-[28px] sm:text-[42px] font-bold leading-[1.15] tracking-tight mb-5" style={{ color: '#E0E7FF' }}>
                {copy.hero.h1[lang]}
                <br />
                <em className="not-italic" style={{ color: '#6D28D9' }}>{copy.hero.h1_em[lang]}</em>
              </h1>
              <p className="text-[15px] sm:text-[16px] leading-[1.7] mb-7 max-w-[520px]" style={{ color: 'rgba(224,231,255,0.7)' }}>{copy.hero.sub[lang]}</p>
              ...
              <p className="font-display text-[13px] italic" style={{ color: 'rgba(224,231,255,0.3)' }}>{copy.hero.verse[lang]}</p>
            </div>
            ...
            <div className="hidden lg:flex relative justify-center items-center">
              ...
              <div className="relative z-10 w-[280px]" style={{ filter: 'drop-shadow(0 20px 40px rgba(0,0,0,0.35))' }}>
                <div className="rounded-[32px] overflow-hidden" style={{ background: '#0F0A18', border: '3px solid rgba(109,40,217,0.3)', boxShadow: 'inset 0 0 0 1px rgba(224,231,255,0.08)' }}>
                  {/* Status bar */}
                  <div className="flex items-center justify-between px-5 pt-3 pb-1" style={{ background: '#0F0A18' }}>
                    <span className="text-[9px] font-semibold" style={{ color: 'rgba(224,231,255,0.5)' }}>9:41</span>
                    <div className="flex gap-1 items-center">
                      <div className="w-3 h-2 rounded-sm" style={{ background: 'rgba(224,231,255,0.4)' }} />
                      <div className="w-1.5 h-1.5 rounded-full" style={{ background: 'rgba(224,231,255,0.4)' }} />
                    </div>
                  </div>
                  {/* App content */}
                  <div className="px-4 pb-5 pt-2 space-y-3" style={{ background: '#FFFFFF', minHeight: '400px' }}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex gap-1">
                        {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                          <span key={l} className="text-[7px] font-bold px-1.5 py-0.5 rounded" style={{ background: l === lang ? '#0F0A18' : 'rgba(15,10,24,0.1)', color: l === lang ? '#E0E7FF' : '#0F0A18' }}>{l}</span>
                        ))}
                      </div>
                    </div>
                    {/* Form fields */}
                    ...
                    <div className="rounded-lg p-2.5" style={{ background: '#fff', border: '1px solid rgba(15,10,24,0.08)' }}>
                      ...
                      <div className="space-y-1">
                        <div className="h-1.5 rounded w-full" style={{ background: '#EEF2FF' }} />
                        <div className="h-1.5 rounded w-5/6" style={{ background: '#E0E7FF' }} />
                        <div className="h-1.5 rounded w-4/5" style={{ background: '#E0E7FF' }} />
                        <div className="h-1.5 rounded w-2/3" style={{ background: '#E0E7FF' }} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="absolute left-0 bottom-12 z-20 flex gap-1.5 rounded-xl px-3 py-2 shadow-lg backdrop-blur-sm" style={{ background: 'rgba(224,231,255,0.92)', border: '1px solid rgba(109,40,217,0.2)' }}>
                {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                  <span key={l} className="text-[10px] font-bold px-2 py-0.5 rounded" style={{ background: l === lang ? '#0F0A18' : 'rgba(15,10,24,0.1)', color: l === lang ? '#E0E7FF' : '#0F0A18' }}>{l}</span>
                ))}
              </div>
              {/* Glow */}
            ...
            {copy.problem.cards.map((c, i) => {
              ...
              return (
                <RevealOnScroll key={i} delay={i * 0.12}>
                  <div className="rounded-xl p-5 h-full" style={{ background: '#E0E7FF', border: '1px solid rgba(15,10,24,0.12)' }}>
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center mb-3" style={{ background: '#EEF2FF' }}>
      ...
      {/* ===== 4. HOW IT WORKS ===== */}
      <section id="how" className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#E0E7FF' }}>
        <div className="max-w-3xl mx-auto">
            ...
            {copy.how.steps.map((step, i) => (
              <RevealOnScroll key={i} delay={i * 0.18}>
                <div className="flex gap-5 py-5" style={{ borderBottom: i < 2 ? '1px solid rgba(15,10,24,0.1)' : 'none' }}>
                  <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 font-display text-xl font-semibold" style={{ background: '#0F0A18', color: '#E0E7FF' }}>{i + 1}</div>
                  <div className="flex-1">
                    ...
                    <span className="inline-block text-[12px] font-medium px-3 py-1 rounded-full" style={{ background: '#EEF2FF', color: '#0F0A18' }}>{step.badge[lang]}</span>
                  </div>
                </div>
              </RevealOnScroll>
            ))}
          ...
          <RevealOnScroll delay={0.3}>
            <div className="max-w-2xl mx-auto">
              <div className="rounded-t-xl overflow-hidden" style={{ background: '#EEF2FF', border: '2px solid rgba(15,10,24,0.15)' }}>
                <div className="flex items-center gap-1.5 px-3 py-2" style={{ background: '#EEF2FF' }}>
                  ...
                  <div className="flex gap-5">
                    <div className="flex-1 space-y-2.5">
                      ...
                      <div className="flex gap-1.5 pt-1">
                        {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                          <span key={l} className="text-[7px] font-semibold px-2 py-0.5 rounded" style={{ background: l === lang ? '#0F0A18' : 'rgba(15,10,24,0.1)', color: l === lang ? '#E0E7FF' : '#0F0A18' }}>{l}</span>
                        ))}
                      </div>
                    ...
                    <div className="flex-1 hidden sm:block">
                      <div className="flex flex-wrap gap-1 mb-2">
                        {[
                          ...
                          <span key={i} className="text-[7px] font-semibold px-1.5 py-0.5 rounded" style={{ background: i === 0 ? '#0F0A18' : 'rgba(15,10,24,0.08)', color: i === 0 ? '#E0E7FF' : '#1E1240' }}>{tab}</span>
                        ))}
                      </div>
                      <div className="rounded-lg p-2.5 space-y-1.5" style={{ background: '#fff', border: '1px solid rgba(15,10,24,0.08)' }}>
                        <div className="h-2 rounded w-3/4" style={{ background: '#EEF2FF' }} />
                        <div className="h-1.5 rounded w-full" style={{ background: '#E0E7FF' }} />
                        <div className="h-1.5 rounded w-full" style={{ background: '#E0E7FF' }} />
                        <div className="h-1.5 rounded w-5/6" style={{ background: '#E0E7FF' }} />
                        <div className="h-1.5 rounded w-full" style={{ background: '#E0E7FF' }} />
                        <div className="h-1.5 rounded w-2/3" style={{ background: '#E0E7FF' }} />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <RevealOnScroll>
            <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-6 items-center">
              {/* Input card */}
              <div className="rounded-2xl p-6 text-center" style={{ background: '#E0E7FF', border: '1px solid rgba(15,10,24,0.12)' }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {copy.multiformat.outputs.map((out, i) => (
                  <div key={i} className="rounded-xl px-3 py-3 flex items-center gap-2 transition-all hover:scale-[1.02]" style={{ background: '#E0E7FF', border: '1px solid rgba(15,10,24,0.08)' }}>
      ...
      <section id="features" className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#E0E7FF' }}>
        <div className="max-w-4xl mx-auto">
      ...
      <section className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#0F0A18' }}>
        <div className="max-w-3xl mx-auto">
          <h2 className="font-display text-[22px] sm:text-[32px] font-semibold text-center mb-8 leading-tight" style={{ color: '#E0E7FF' }}>{copy.vs.h2[lang]}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
              <p className="text-[12px] font-semibold uppercase tracking-wide mb-4" style={{ color: 'rgba(224,231,255,0.45)' }}>{copy.vs.them[lang]}</p>
              <div className="space-y-2.5">
                {copy.vs.xItems.map((item, i) => (
                  <div key={i} className="flex items-start gap-2.5 text-[14px]">
                    <span className="text-base shrink-0 mt-0.5" style={{ color: 'rgba(224,231,255,0.3)' }}>✗</span>
                    <span style={{ color: 'rgba(224,231,255,0.55)' }}>{item[lang]}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2.5">
                {copy.vs.checkItems.map((item, i) => (
                  <div key={i} className="flex items-start gap-2.5 text-[14px]">
                    ...
                    <span className="font-medium" style={{ color: 'rgba(224,231,255,0.9)' }}>{item[lang]}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ===== 8. TESTIMONIALS ===== */}
      <section className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#E0E7FF' }}>
        <div className="max-w-3xl mx-auto">
            ...
            {copy.testimonials.itemsByLang[lang].map((t, i) => (
              <div key={i} className="rounded-2xl p-6" style={{ background: '#FFFFFF', border: '1px solid rgba(15,10,24,0.08)', boxShadow: '0 1px 4px rgba(15,10,24,0.04)' }}>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-semibold shrink-0 tracking-wide" style={{ background: '#EEF2FF', color: '#0F0A18' }}>{t.initials}</div>
                  <div>
      ...
      <section className="py-14 sm:py-16 px-5 sm:px-8" style={{ background: '#FFFFFF' }}>
        <div className="max-w-4xl mx-auto">
          <div className="rounded-2xl p-6 sm:p-10" style={{ background: '#E0E7FF', border: '1px solid rgba(15,10,24,0.1)' }}>
            <div className="flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="flex-1">
                ...
                <Link to="/cadastro" className="inline-flex items-center gap-2 text-[14px] font-semibold px-5 py-2.5 rounded-lg transition-all hover:scale-[1.02]" style={{ background: '#0F0A18', color: '#E0E7FF' }}>
            ...
            {regionLoading ? (
              [0,1,2,3].map(i => (
                <div key={i} className="rounded-xl p-5 flex flex-col gap-3 animate-pulse" style={{ background: '#FFFFFF', border: '1px solid rgba(15,10,24,0.12)' }}>
                  <div className="h-4 w-16 rounded" style={{ background: '#EEF2FF' }} />
                  <div className="h-9 w-24 rounded" style={{ background: '#EEF2FF' }} />
                  <div className="h-3 w-20 rounded" style={{ background: '#EEF2FF' }} />
                  <div className="space-y-2 mt-4">
                    {[0,1,2,3].map(j => <div key={j} className="h-3 w-full rounded" style={{ background: '#EEF2FF' }} />)}
                  </div>
                  <div className="h-10 w-full rounded-lg mt-auto" style={{ background: '#EEF2FF' }} />
                </div>
              ))
            ) : (
              pricingPlans.map((plan, i) => (
                <RevealOnScroll key={i} delay={i * 0.1}>
                  <div className="rounded-xl p-5 flex flex-col h-full" style={{
                    background: plan.featured ? '#E0E7FF' : '#FFFFFF',
                    border: plan.featured ? '2px solid #0F0A18' : '1px solid rgba(15,10,24,0.12)',
                  }}>
                    {plan.featured && (
                      <span className="inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full mb-2 self-start" style={{ background: '#0F0A18', color: '#E0E7FF' }}>
                    ...
                    <span className="inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full mb-4 self-start" style={{ background: '#EEF2FF', color: '#0F0A18' }}>
                      {plan.capacity[lang]}
                    </span>
                    ...
                    <Link to={plan.planKey ? `/cadastro?plan=${plan.planKey}` : '/cadastro'} className="block text-center text-[14px] font-semibold py-3 rounded-lg transition-transform hover:scale-[1.02]" style={{
                      background: plan.featured ? '#0F0A18' : '#EEF2FF',
      ...
      {/* ===== 11. FAQ ===== */}
      <section className="py-14 sm:py-18 px-5 sm:px-8" style={{ background: '#E0E7FF' }} itemScope itemType="https://schema.org/FAQPage">
      ...
      <section className="py-16 sm:py-20 px-5 text-center safe-area-bottom" style={{ background: '#0F0A18' }}>
        <div className="max-w-2xl mx-auto">
          <h2 className="font-display text-[24px] sm:text-[38px] font-semibold leading-tight mb-4" style={{ color: '#E0E7FF' }}>
            {copy.ctaFinal.h2_1[lang]}<br /><em>{copy.ctaFinal.h2_em[lang]}</em>
          </h2>
          <p className="text-[16px] leading-[1.7] mb-8" style={{ color: 'rgba(224,231,255,0.75)' }}>{copy.ctaFinal.sub[lang]}</p>
          <Link to="/cadastro" className="inline-block text-[16px] font-semibold px-9 py-4 rounded-xl mb-4 transition-transform hover:scale-[1.02]" style={{ background: '#E0E7FF', color: '#0F0A18' }}>{copy.ctaFinal.cta[lang]}</Link>
          <br />
          <span className="text-[13px] font-medium" style={{ color: 'rgba(224,231,255,0.5)' }}>{copy.ctaFinal.tags[lang]}</span>
          <p className="font-display text-[14px] italic mt-6" style={{ color: 'rgba(224,231,255,0.35)' }}>{copy.ctaFinal.verse[lang]}</p>
        </div>
      </section>
      ...
      <footer className="py-8 px-5 sm:px-8" style={{ background: '#0F0A18' }}>
        <div className="max-w-4xl mx-auto flex flex-col items-center gap-5">
          <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="font-display text-lg" style={{ color: 'rgba(224,231,255,0.7)' }}>
              Living <span style={{ color: '#6D28D9' }}>Word</span>
            </span>
            <div className="flex items-center gap-5">
              <span className="text-[13px] font-medium" style={{ color: 'rgba(224,231,255,0.4)' }}>{lang === 'PT' ? 'Privacidade' : lang === 'EN' ? 'Privacy' : 'Privacidad'}</span>
              <span className="text-[13px] font-medium" style={{ color: 'rgba(224,231,255,0.4)' }}>{lang === 'PT' ? 'Termos' : lang === 'EN' ? 'Terms' : 'Términos'}</span>
              <span className="text-[13px] font-medium" style={{ color: 'rgba(224,231,255,0.4)' }}>{lang === 'PT' ? 'Contato' : lang === 'EN' ? 'Contact' : 'Contacto'}</span>
            </div>
            <div className="flex gap-3">
              {(['PT', 'EN', 'ES'] as L[]).map((l) => (
                <button key={l} onClick={() => setLang(l)} className="text-[13px] font-semibold" style={{ color: l === lang ? '#6D28D9' : 'rgba(224,231,255,0.35)' }}>{l}</button>
              ))}
            </div>
          </div>
