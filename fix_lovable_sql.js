...
// Tables that we expect already exist in our DB and need ALTER TABLE ADD COLUMN instead of just CREATE TABLE IF NOT EXISTS
const existingTables = ['profiles', 'materials', 'devotionals', 'generation_logs'];

let alterStatements = '-- =====================================================\n';
alterStatements += '-- 0. ADD MISSING COLUMNS TO EXISTING TABLES\n';
alterStatements += '-- =====================================================\n\n';

for (const tableName of existingTables) {
  const tableRegex = new RegExp(`CREATE TABLE IF NOT EXISTS public\\.${tableName}\\s*\\(([\\s\\S]*?)\\);`, 'g');
  const match = tableRegex.exec(inputSql);
  
  if (match) {
    const columnsBlock = match[1];
    const columns = columnsBlock.split(',\n').map(c => c.trim()).filter(c => c.length > 0 && !c.startsWith('id uuid')); // skip id or constraints
    
    for (const col of columns) {
      // Examples: "full_name text NOT NULL DEFAULT ''"
      // Wait, primary keys are "id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY" which we skipped.
      if (col.startsWith('UNIQUE') || col.startsWith('PRIMARY KEY') || col.startsWith('FOREIGN KEY')) continue;
      
      const colNameMatch = col.match(/^([a-zA-Z0-9_]+)/);
      if (colNameMatch) {
         const colName = colNameMatch[1];
         // Skip columns we know we definitely already have
         if (['id', 'user_id', 'created_at', 'updated_at', 'generations_used', 'generations_limit'].includes(colName) && tableName === 'profiles') continue;
         if (['id', 'user_id', 'created_at', 'updated_at', 'title', 'type', 'content', 'passage'].includes(colName) && tableName === 'materials') continue;
         if (['id', 'user_id', 'created_at', 'feature'].includes(colName) && tableName === 'generation_logs') continue;
         
         // In Postgres, adding a NOT NULL column to a table with rows requires a DEFAULT.
         // Most Lovable strings have DEFAULT '', numbers have DEFAULT 0.
         // If a column is NOT NULL but lacks DEFAULT, it will block if the table has data.
         // Let's strip "NOT NULL" if there is no default, or just let ALTER TABLE use the exact definition.
         let safeColDef = col;
         
         // A safe pattern is to add the column without NOT NULL if the table has data, but since we are just forcing the Lovable schema, we keep it as is, Supabase will run it.
         alterStatements += `ALTER TABLE public.${tableName} ADD COLUMN IF NOT EXISTS ${safeColDef};\n`;
      }
    }
  }
}

const finalSql = alterStatements + '\n\n' + inputSql;
fs.writeFileSync('/Users/severinobione/PALAVRA VIVA/supabase/migrations/016_lovable_schema_sync.sql', finalSql);
console.log('Fixed SQL saved to 016_lovable_schema_sync.sql');
