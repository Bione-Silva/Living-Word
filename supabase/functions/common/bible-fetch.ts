/**
 * Shared Bible verse fetching logic.
 * Used by fetch-bible-verse edge function and other generators.
 */
...
async function fetchFromWldeh(passage: string, lang: "pt_AA" | "pt_ACF"): Promise<string | null> {
  try {
    ...
    const bookSlug = bookNameMap[parsed.bookCode] ?? parsed.bookCode.toLowerCase()
    ...
    if (versesStr && data.verses) {
      ...
      const filtered = data.verses
        .filter((v: { verse: number }) => v.verse >= startVerse && v.verse <= (endVerse || startVerse))
