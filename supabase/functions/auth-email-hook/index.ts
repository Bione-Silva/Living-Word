...
// Configuration
const SITE_NAME = "living-word"
const SENDER_DOMAIN = "notify.livingwordgo.com"
const ROOT_DOMAIN = "livingwordgo.com"
const FROM_DOMAIN = "livingwordgo.com" // Domain shown in From address (may be root or sender subdomain)
...
const SAMPLE_PROJECT_URL = "https://living-word.lovable.app"
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  const rawLang =
    (payload.data as any)?.user?.user_metadata?.language ??
    (payload.data as any)?.user?.user_metadata?.locale ??
    (payload.data as any)?.metadata?.language ??
    'pt'
  const lang = normalizeLang(rawLang)
  console.log('Received auth event', { emailType, email: payload.data.email, lang, run_id })
